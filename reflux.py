...
Updated Version that fixes the NFE Budget bug and added Phased EPC with Cache-Only Mode.
=====================================================================
...
Losses:
    ...
    L_epc1:  ||v_θ(x_1, 1) - (x_1 - x_0)||^2       endpoint anchor at t=1 (optional, anchor_x_1)
...
def compute_nfe_budget_info(cfg: ReFluxConfig) -> dict:
    """Compute NFE costs per step and max steps under the budget.

    Coupling phase (full mode):
        Coupling:  ODE forward on M = oversample * N noise vectors, K steps each → M*K NFEs
        Training:  CFM only (random t) → N NFEs
    
    Cache-only mode (after crystallization):
        Coupling:  0 (cached noise used directly)
        Training:  CFM + EPC0 (+ optional EPC1) → N*2 or N*3 NFEs
    """
    N = cfg.batch_size
    M = N * cfg.noise_oversample  # total candidates per step
    K = cfg.cost_num_steps

    # Coupling NFEs: M total candidates × K ODE steps
    coupling_nfe_per_step = M * K

    # Training NFEs differ by phase:
    #   Coupling phase: CFM only (1 fwd pass per sample)
    #   Cache-only phase: CFM + EPC0 (+ EPC1 if anchor_x_1)
    coupling_phase_training_nfe = N  # CFM only
    cache_only_training_nfe = N * (3 if cfg.anchor_x_1 else 2)  # CFM + EPC

    # Total NFE per step
    total_nfe_per_step = coupling_nfe_per_step + coupling_phase_training_nfe
    cache_only_nfe_per_step = cache_only_training_nfe  # no coupling, but EPC added

    max_steps = cfg.nfe_budget // total_nfe_per_step

    return {
        'coupling_nfe_per_step': coupling_nfe_per_step,
        'coupling_phase_training_nfe': coupling_phase_training_nfe,
        'cache_only_training_nfe': cache_only_training_nfe,
        'total_nfe_per_step': total_nfe_per_step,
        'cache_only_nfe_per_step': cache_only_nfe_per_step,
        'max_steps': max_steps,
...
print(f"{'='*60}")
print(f"  NFE budget:          {config.nfe_budget:,} NFEs")
...
print(f"  Training NFE/step:   {nfe_info['coupling_phase_training_nfe']:,} (coupling phase, CFM only) / {nfe_info['cache_only_training_nfe']:,} (cache-only, CFM+EPC)")
print(f"  Total NFE/step:      {nfe_info['total_nfe_per_step']:,} (full) / {nfe_info['cache_only_nfe_per_step']:,} (cache-only)")
...
def train_reflux(model, ema, source_config, dataloader, num_dataset_samples, config):
    ...
    total_nfe_per_step = nfe_info['total_nfe_per_step']
    cache_only_nfe_per_step = nfe_info['cache_only_nfe_per_step']
    ...
    print(f"  Lambda EPC:        {config.lambda_epc}")
    print(f"  NFE budget:        {config.nfe_budget:,}")
    print(f"  Total NFE/step:    {total_nfe_per_step:,} (full) / {cache_only_nfe_per_step:,} (cache-only)")
    print(f"  Max opt steps:     {max_steps:,} (before cache-only savings)")
    print(f"  Noise cache:       {noise_cache.num_samples:,} entries")
    print(f"  EPC anchors:       t=0 (x_0){' and t=1 (x_1)' if config.anchor_x_1 else ' only'}")
    ...
    step = start_step
    pbar = tqdm(desc="ACID ReFlux", dynamic_ncols=True, initial=start_step-1, total=max_steps)
    while step <= max_steps:
        ...
        if not cache_only_mode:
            ...
            step_nfe = total_nfe_per_step
        else:
            # --- Cache-only mode: just use cached noise, no ODE needed ---
            x_0 = cached_x0
            chosen_costs = None
        ...
        mean_coupling_cost = chosen_costs.mean().item() if chosen_costs is not None else 0.0

        # --- Cache crystallization: rolling average over last P steps ---
        P = config.cache_patience
        if not cache_only_mode and len(cache_use_rate_history) >= P and \
                np.mean(cache_use_rate_history[-P:]) >= config.crystallization_threshold:
            ...
            tqdm.write(
                ...
                f"      NFE/step drops from {total_nfe_per_step:,} → "
                f"{cache_only_nfe_per_step:,} (training only, EPC enabled)\n"
        ...
        # --- B. EPC anchor at t=0 (only after cache crystallization) ---
        if cache_only_mode:
            ...
            loss_epc0 = torch.tensor(0.0)

        # --- C. EPC anchor at t=1 (optional, only after cache crystallization) ---
        if cache_only_mode and config.anchor_x_1:
        ...
        P = config.cache_patience
        avg_use = np.mean(cache_use_rate_history[-P:]) if cache_use_rate_history else cache_use_rate
        pbar.set_postfix({
            'tot': f'{loss_total:.4f}',
        ...
        if step % config.eval_interval == 0:
            ...
            if cos_sim > best_cos_sim:
                ...
                torch.save(model.state_dict(), f"{config.output_dir}/reflux_best.pt")
        ...
        if step % config.save_interval == 0:
            ...
            torch.save(checkpoint_data, f"{config.output_dir}/reflux_step{step:06d}.pt")
            # Save latest checkpoint for resumption
            torch.save(checkpoint_data, f"{config.output_dir}/reflux_latest.pt")
            # Save cache
    ...
    last_step = step - 1  # step was incremented past the last completed step
    pbar.close()
    ...
    final_checkpoint = {
        'model': model.state_dict(),
        'ema': ema.state_dict(),
        'step': last_step,  # Use actual last step
        'cumulative_nfe': cumulative_nfe,
        'best_cos_sim': best_cos_sim,
        'config': source_config,
        'epc_cache_config': config,
    ...
    torch.save(final_checkpoint, f"{config.output_dir}/reflux_final.pt")
    torch.save(final_checkpoint, f"{config.output_dir}/reflux_latest.pt")
    noise_cache.save(f"{config.output_dir}/noise_cache_latest.pt")
    noise_cache.save(f"{config.output_dir}/noise_cache_final.pt")

    if last_step >= config.ema_start_step:
        ema.apply_shadow()
    visualize_samples_grid(model, config, last_step, str(samples_dir))
    if last_step >= config.ema_start_step:
        ema.restore()
    ...
    print(f"  Total NFE:          {cumulative_nfe:,} / {config.nfe_budget:,} "
